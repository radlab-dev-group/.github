...
def convert_line(line):
    ...
    labels = [key for key, value in data.items() if key not in exclude_keys and value == 1]
    
    return json.dumps({"text": text, "labels": labels}, ensure_ascii=False)

def main():
    parser = argparse.ArgumentParser(description="Convert raw Clarin PL TwitterEmo dataset to labels format.")
    parser.add_argument("--input", required=True, help="Path to the input raw jsonl file")
