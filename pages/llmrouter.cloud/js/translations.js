export const translations = {
  pl: {
    ...
    'features.protection.desc': 'Platforma automatycznie wykrywa i maskuje wrażliwe dane (PESEL, NIP, KRS, REGON, e‑maile, numery kont NRB, karty płatnicze, adresy IP itp.) oraz wykonuje kontrole guardrails przed przekazaniem żądania do dostawców LLM. Zarówno etap maskowania, jak i guardrails generują niezmienialne, zaszyfrowane logi audytowe (GPG), zapewniając pełną rozliczalność.',
    'features.loadbalancing.title': 'Inteligentne równoważenie obciążenia',
    'features.loadbalancing.desc': 'Pięć konfigurowalnych strategii: <code>balanced</code>, <code>weighted</code>, <code>dynamic_weighted</code>, <code>first_available</code> oraz zoptymalizowana <code>first_available_optim</code> (re-use host). Optymalizacja wykorzystania zasobów i minimalizacja latencji poprzez inteligentny wybór dostawcy.',
    'features.routing.title': 'Zunifikowany routing lokalny i chmurowy',
    'features.routing.desc': 'Pojedyncze REST API obsługujące lokalnych dostawców (Ollama, vLLM, LM Studio) oraz usługi chmurowe (OpenAI, Anthropic, Google). Obsługa rodziny modeli Bielik (Speakleash), automatyczna konwersja protokołów i płynne przełączanie między dostawcami w czasie rzeczywistym.',
    ...
    'usecases.local.solution': '<strong>Rozwiązanie:</strong> LLM Router zapewnia zunifikowane zarządzanie wszystkimi lokalnymi wdrożeniami (vLLM, Ollama, LM Studio). Uruchamiaj modele takie jak Bielik-11B na własnym sprzęcie, dystrybuuj obciążenie między wiele instancji i korzystaj z systemu Keep-Alive, aby uniknąć opóźnień przy pierwszym żądaniu. Zachowaj dane całkowicie w swojej infrastrukturze.',
  ...
  en: {
    ...
    'features.protection.desc': 'The platform automatically detects and masks sensitive data (PESEL, NIP, KRS, REGON, emails, bank accounts, credit cards, IP addresses, etc.) and runs guard‑rail checks before forwarding requests to LLM providers. Both masking and guard‑rail stages emit tamper‑evident, encrypted audit logs (GPG) for full accountability.',
    'features.loadbalancing.title': 'Intelligent Load Balancing',
    'features.loadbalancing.desc': 'Five configurable load balancing strategies: <code>balanced</code>, <code>weighted</code>, <code>dynamic_weighted</code>, <code>first_available</code>, and <code>first_available_optim</code> (re-use host). Optimize resource utilization and minimize latency with intelligent provider selection.',
    'features.routing.title': 'Unified Local & Cloud Routing',
    'features.routing.desc': 'Single REST API supporting local providers (Ollama, vLLM, LM Studio) and cloud services (OpenAI, Anthropic, Google). Support for Bielik (Speakleash) model family, automatic protocol conversion, and seamless real-time provider switching.',
    ...
    'usecases.local.solution': '<strong>Solution:</strong> LLM Router provides unified management for all your local model deployments (vLLM, Ollama, LM Studio). Run models like Bielik-11B on your own hardware, distribute load across multiple instances, and use the Keep-Alive system to avoid first-request latency. Keep data entirely within your infrastructure.',
